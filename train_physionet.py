...
from utils.dataload import load_bciciv2a_data_single_subject,load_physionet_data_single_subject,load_bci2b_data_single_subject,load_physionet_data_single_subject_binary
...
def start_run(args):
    ...
    if args.data_type == 'bci2a':
        train_X, train_y, test_X, test_y = load_physionet_data_single_subject_binary(args.data_path,subject_id=args.id)
    elif args.data_type == 'physionet':
        # args.data_path = ''
        args.channel_num = 64
        args.n_class = 4
        args.out_chans = 64   
        args.sampling_rate = 160 
        args.window_length = 80
        args.window_padding = 50    
        # train_X, train_y, test_X, test_y = load_physionet_data_single_subject(args.data_path,subject_id=args.id)
        data,label = load_physionet_data_single_subject_binary(args.data_path,subject_id=args.id)
        # data,label = load_physionet_data_single_subject(args.data_path,subject_id=args.id)
        ratio = args.train_ratio
        N,C,T = data.shape
        train_X = data[0:int(N*ratio)]
        train_y = label[0:int(N*ratio)]
        test_X = data[int(N*ratio):]
        test_y = label[int(N*ratio):]
    ...
    if 'l' == args.spatial_adj_mode:
        Adj = torch.tensor(load_adj(args.data_type), dtype=torch.float32)
    ...
    # model_classifier = torch.nn.DataParallel(model_classifier.cuda(gpus[0]), device_ids=gpus, output_device=gpus[0])
...
if __name__ == "__main__":
    ...
    parser.add_argument('-train_ratio', type=float, default=0.8, help='Train dataset for physionet.')
    ...
    parser.add_argument('-k_time',type=int,default=1,help='The layer of time_GCN embedding')
    ...
    acc_list = []
    # sub_list = [3,7,15,22,35,42,55,70,86,101] # 四分类个体
    sub_list = [1,3,7,8,22,42,53,70,86,94] # 二分类个体
    for i in sub_list:
        args_.data_type = 'physionet'
        args_.id = i
        args_.spatial_GCN = True
        args_.time_GCN = True
        start_run(args_)
