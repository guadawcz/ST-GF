...
from utils.dataload import load_bciciv2a_data_single_subject,load_physionet_data_single_subject,load_bci2b_data_single_subject,load_bciciv2a_data_subject
...
def start_run(args):
    ...
    if args.data_type == 'bci2a':
        for i in range(1,10):
            args.id = i
            if i==1:
                data,label = load_bciciv2a_data_subject(args.data_path,subject_id=args.id)
            else:
                t1,t2 = load_bciciv2a_data_subject(args.data_path,subject_id=args.id)
                data = torch.cat([data,t1],dim=0)
                label = torch.cat([label,t2],dim=0)
        
        t1,t2 = load_bciciv2a_data_subject(args.data_path,subject_id=1)
        data = torch.cat([data,t1],dim=0)
        label = torch.cat([label,t2],dim=0)
        train_X = data
        train_y = label
        test_X,test_y = load_bciciv2a_data_subject(args.data_path,subject_id=2)
        ...
        args.data_path = '/disk1/zhaokui/models/IFNet-main/MyModel/standard_2b_data'
        args.channel_num = 3
        args.n_class = 2
        args.out_chans = 8
...
if __name__ == "__main__":
    ...
    parser.add_argument('-data_path',type=str,default='/disk1/wangxuhui/data/BCIC 2a',help='The data path file.')
    ...
    parser.add_argument('-father_path',type=str,default='/disk1/wangxuhui/project/STGENet/saved_information')
